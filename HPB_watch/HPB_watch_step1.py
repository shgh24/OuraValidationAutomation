...
for id in IDS:
    ...
    df_temp = df_temp.loc[df_temp.index.repeat(df_temp.intervaldurationmins*2)]

    participant = id.replace('NUS3', 'NUS3_')
    match = df[(df['Night'] == 1)
    ...
    df_temp['timestamp'] = pd.to_datetime(
        df_temp['intervalstartdatetime'], utc=True)
