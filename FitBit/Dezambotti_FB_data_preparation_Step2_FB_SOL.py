...
staging_file_path = "/Volumes/CSC5/SleepCognitionLab/Tera2b/Experiments/OuraValidation/Oura3/data_raw/Sleep_Staging/Fitbit/SOL_Fitbit/"
...
for Night in nights:
    ...
    Consesnus_Scores = glob.glob(os.path.join(
        staging_file_path, "PSG", f"NUS*{Night}*.csv"))
    ...
    for PSG_Scores in Consesnus_Scores:
        ...
        if matching_files:
            ...
            for FB in matching_files:
                ...
                # data_type = df_FB.iloc[:, 0].unique()
                # print(
                #     f"Data type for {subject1}_{night}: {data_type}")

                if abs(len(df_PSG)-len(df_FB)) <= 200:
                    # Sanity check
