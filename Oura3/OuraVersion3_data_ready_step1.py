...
(Oura Ring provided the 2nd and 3rd batch of data in the new format and that's why the script named OuraVersion3 and this scripts will generate new files in the format of first batch file as below:
...
def save_sleep_period_data(csv_file, files, save_path2):
    ...
    for day in unique_days:
        ...
        sleep_period_data = []
        for row in timestamp_data:
            timestamp = datetime.fromisoformat(row[1]).replace(
        ...
        if sleep_period_data:
            # sleep_period_data = np.delete(sleep_period_data, 0, 1)
...
def save_to_csv(day, data, files, save_path):
    ...
    with open(filename, 'w', newline='') as file:
        ...
        writer.writerow(["name", "timestamp", "predicted"])  # Header row
