...
for sheet_name in sheet_names:
    ...
    for i in range(len(ind)-1):
        ...
        # pd.Timestamp(data['Date'][0])
        # pd.to_datetime(data['Date'][2])

        # start_date = end_date - \
        #     pd.DateOffset(days=len(data[start_idx:end_idx-1]))

        X = data['Date'][2]
        newdatetime = end_date.replace(hour=X.hour, minute=X.minute)

        Dates = data.loc[start_idx:end_idx-1].iloc[:, 0:2]
...
def change_time(end_date, Dates):
